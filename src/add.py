...
if __name__ == "__main__":
    ...
    log_filename = f"logs/add_log_{timestamp}.txt"
    ...
    destination = f"circuit/add_{timestamp}.png"
    drawing = addCircuit.draw(save_to=destination)
    logger.info("Circuit drawing saved to %s", destination)

    mlir_destination = f"circuit/add_{timestamp}.mlir"
    with open(mlir_destination, "w", encoding="utf-8") as f:
        f.write(addCircuit.mlir)
    logger.info("MLIR saved to %s", mlir_destination)

    ciphertexts = addCircuit.encrypt(plaintext_a, plaintext_b)
    ...
    st = time.time_ns()
    ciphertext_result = addCircuit.run(ciphertext_a, ciphertext_b)
    ...
    logger.info("Time to compute the addition: %.6fs", pt)

    result = addCircuit.decrypt(ciphertext_result)
